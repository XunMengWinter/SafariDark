...
- `IMPLEMENTED.md`：当前代码已经实现的能力、入口文件、限制和验证记录。
...
- Safari 宠物属于本地页面能力，图片、状态和交互都必须在扩展本地完成，不加载远程资源。
...
- Safari 宠物默认开启，只在可注入网页的 top frame 注入，资源保持本地。
- 宠物 DOM 必须使用独立、可识别的宿主节点，避免被媒体保真、对比度修复或页面暗色滤镜误处理。
- 宠物设置变化要同步检查 `content.js`、`popup.js`、`manifest.json`、`SPEC.md` 和 `IMPLEMENTED.md`。
